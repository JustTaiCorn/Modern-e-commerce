...
export function ProductDetails({ product }: ProductDetailsProps) {
  ...
  // ponytail: Chuẩn hóa mảng ảnh hỗ trợ cả object Prisma lẫn string legacy
  ...
  const handleAddToCart = async () => {
    ...
    if (!variantId) {
      toast({
        ...
        description: 'Vui lòng chọn biến thể sản phẩm trước khi thêm vào giỏ.',
        variant: 'destructive',
      });
      return;
    }
    ...
    <div className="container mx-auto py-10 md:px-2 lg:px-1">
      <div className="grid grid-cols-2 gap-x-12 sm:gap-x-8 md:gap-x-6 md:grid-cols-1 lg:gap-y-6">
              ...
              {images.map((image, index) => (
                <motion.button
                  ...
                  className={`relative w-20 sm:w-16 aspect-square rounded-md overflow-hidden border-[1px] transition-all sm:flex-shrink-0
                    ${index === currentImageIndex ? ' border-black dark:border-white' : 'border-transparent'}`}
        ...
        <div className="space-y-6 sm:space-y-4">
          ...
          {/* ponytail: Bộ chọn Biến thể (Variants Selector) */}
          {product.variants && product.variants.length > 1 && (
            <div className="space-y-2">
              <h3 className="font-medium text-sm">Biến thể (Variants):</h3>
              <div className="flex flex-wrap gap-2">
                {product.variants.map((v) => {
                  ...
                  return (
                    ...
                    >
                      {v.sku} ({formatPrice(Number(v.price))})
                    </button>
                  );
                })}
              </div>
            ...
            {isOutOfStock ? (
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {Array.from(
                      { length: Math.min(currentStock, 10) },
                      (_, i) => i + 1,
                    ).map((num) => (
